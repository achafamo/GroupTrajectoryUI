...
;t (symmetric for y). Let n be the number of entities and tau the
;number of timestamps. Then the input file format is:
;
;--------- BOF ------
;n 
;tau
...
turtles-own [
  ...
  was-in-border      ;; wether or not the trajectory was in the border the previous time-stamp
  prev-angle         ;; previous turning angle
]

globals [ 
  trace  ;;; enable tracing and storing the trajectories
]
...
to setup
  ...
  ask patches [ if in-border? [set pcolor grey ]]
  set trace False
...
to save 
  let f "netlogo_trajectories.txt"
  file-delete f  ;; make sure to delete the thing if it exists
  file-open f
  file-print count turtles
  file-print ([length trajectory] of one-of turtles)
  ask turtles [
    foreach trajectory [file-print ?]
  ]
  file-close
end
...
to go  
  ...
  ; repeat (dist / 0.2) [ ask turtles [move 0.2] display ]
  ;; for greater efficiency, at the expense of smooth
  ;; animation, substitute the following line instead:
  ask turtles [ move dist ]
  tick
  if trace [
    ask turtles [store-position]  
  ]
end
...
to move [d]  ; turtle procedure; move the turtle a distance d   
  let in-border ([in-border?] of patch-here)
  
  ;; random angle change
  let rmax 10
  let rangle ((random-float rmax) - (rmax / 2))
  
  ifelse in-border [
    ifelse not was-in-border [  ;; just entered the border
                                ;; get a turning angle 
      let angle ((turning-direction patch-here) * random 45)
      ;; turn and store the angle
      rt angle 
      set prev-angle angle        
    ] [ ;; I was already in the border
      let angle (prev-angle * random-float 1.2)
      rt angle   
      set prev-angle angle
      ;; if we still cannot move rotate even further! ]
      while [patch-ahead d = nobody] [ 
        rt 40
      ]
    ]
  ] [
    ;; if we are not in the border make the random angle change bigger
    set rangle (1.5 * rangle)
  ]    
  set was-in-border in-border
  ...
  ;; determine in which direction we have to turn. Returning 1 means turning right
  ;; returing -1 means turning left 
to-report turning-direction [p]
  if [in-right-border?]  of p and going-NE? [ report -1 ] 
  if [in-right-border?]  of p and going-SE? [ report  1 ] 
  if [in-bottom-border?] of p and going-SE? [ report -1 ] 
  if [in-bottom-border?] of p and going-SW? [ report  1 ] 
  if [in-left-border?]   of p and going-SW? [ report -1 ] 
  if [in-left-border?]   of p and going-NW? [ report  1 ] 
  if [in-top-border?]    of p and going-NW? [ report -1 ] 
  if [in-top-border?]    of p and going-NE? [ report  1 ]   
  
  report 1
end
  
to-report in-border?
  report in-bottom-border? or in-top-border? or in-left-border? or in-right-border?
end
  
to-report in-bottom-border?
  report pycor < borderwidth
end
  
to-report in-top-border?
  report pycor > world-height - borderwidth
end
  
to-report in-left-border?
  report pxcor < borderwidth
end
  
to-report in-right-border?
  report pxcor > world-width - borderwidth
end
  
  ;; helper procedure to determine the direction
to-report in-angle [l h]  
  report ifelse-value (l <= h) [heading >= l and heading < h] [heading >= l or heading < h]
end  
  
to-report going-NE?  ;; 
  report in-angle 0 90
end
  
to-report going-SE? 
  report in-angle 90 180
end
  
to-report going-SW? 
  report in-angle 180 270
end
  
to-report going-NW? 
  report in-angle 270 0
end
...
to pick-starting-pos 
  setxy random-xcor random-ycor 
;  let i 1
;  ; make sure initially everybody is separated
;  while [any? other turtles in-radius (2 * epsilon) and i < 10000] [
;    setxy random-xcor random-ycor 
;    set i (i + 1)
;  ]
;  if any? other turtles in-radius epsilon [error "FAILED to pick suitable starting position" ]
  
  set was-in-border false  
end
  
  
to store-position ;; turtle procedure, add the current position to the trajectory
  set trajectory lput pos trajectory
end
  
to-report pos ;; ; turtle proc
