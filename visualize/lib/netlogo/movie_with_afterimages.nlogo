;let x_i_t denote the x-coordinate of entity i at time
;(symmetric for y). Let n be the number of entities,  tau the
;number of timestamps, and m the number of groups. 
; Then the input file format is:
;
;--------- BOF ------
;n tau m
;x10 y10 t10 
...
;..  .. 
;tntau xntau yntau 
;g1size g1start g1end
;g2size g2start g2end
;g1e1 g1e2 g1e3 g1e4 .....
;g2e1 g1e2 g2e3 g2e4 .....
;..
;gmsize gmstart gmend
;gme1 gme2 gme3 ...
;--------- EOF----------------

breed [entities entity]
breed [afterimages afterimage]

links-own [
  gids  ;; a list of groupids that are currently active and hence lead to this link  
        ;; the groups are ordered according to their activation style i.e.
        ;; if the first group deactivates, the next group is used to set link styles etc
]

turtles-own [
  trajectory         ;; the coordinates of the trajectory this entity has travelled so far. 
                     ;;  each coordinate is a list of length 2: [xcord, ycoord]
  tid                ;; trajectory id
]

globals [ 
  groups 
  activeGroups ;; the groups that are currently active
  n 
  tau 
  m
  recording ;; boolean to keep track if we are recording stuff
  maxgroupsize
  mingroupsize
]
...
to read_in 
  ...
  while [ i <= n ] [
      create-entities 1 [
        ...
        set tid i
        while [ j < tau ] [
          append-tp file-read file-read file-read
          set j (j + 1) 
        ]
      ]
      set i (i + 1)
  ...
  while [ i <= m ] [
    ...
    append-group i sz startt endt (entities with [member? tid xs])
    set i (i + 1)
  ]
    
  file-close
  
  set mingroupsize min (map groupSize groups)
  set maxgroupsize max (map groupSize groups)
  
end

to-report useAfterImage
  report (afterImageTime > 0)
end
...
to append-group [g sz s t xs]
  let ls buildGroupLinks xs
  set groups lput (list g sz s t xs ls) groups
end

;; adds a given TP to the trajectory
to append-tp [t x y]
  set trajectory lput (list t x y) trajectory
end  
...
to-report groupSize [g]
  report item 1 g
end

to-report groupStart [g]
  report item 2 g
end

to-report groupEnd [g]
  report item 3 g
end

to-report groupMembers [g]
  report item 4 g
end

to-report groupLinks [g]
  report item 5 g  
end

to-report buildGroupLinks [members]
  ;; get all links
  let xs link-set no-links
  ask members [
    let x self
    ask other members [
      create-link-with x [ set gids (list ) ]
      let y self
      set xs (link-set xs (link ([who] of x) ([who] of y)))
...
to record
  movie-start "/home/frank/tmp/out.mov"
  set recording True
  play
  set recording False
  movie-close
...
to init
  reset-ticks
  set activeGroups (list )
  clear-drawing
  
  ask turtles [ 
    let initpos (first  trajectory)
    let snd     (item 1 trajectory)
    set xcor (getX initpos) 
    set ycor (getY initpos)  
    facexy (getX snd) (getY snd)
...
to go  
  ask entities [moveToPosAtTime ticks]
  ask afterimages [moveToPosAtTime (ticks - afterImageTime)]
  showGroup
end

to moveToPosAtTime [t] 
  set t max (list t 0)
  
  let p  (item t trajectory)
  go-to p
  if t < tau - 1 [
    let np (item (t + 1) trajectory)
    facexy (getX np) (getY np)   
  ]  
...
to go-to [p]
  ...
  facexy x y 
  let s (distancexy x y / numsteps)
  repeat numsteps [forward s]

end


to showGroup 
   ;; activate all links of groups that have become active
   foreach (filter [is-active? ? and not member? ? activeGroups] groups) [
     ask groupLinks ? [ activate groupId ? ]
;     ask groupMembers ? [
;       set size 2 + 2 * proportionalSize ?
;     ]
   ]
   ;; deactivate the links that are no longer active
   foreach (filter [not is-active? ?] activeGroups) [
     ask groupLinks ? [ deactivate ]
;    ;; actually we should do this only for inactive group members, but then we'd also need a stack of group sizes to reset them to the appropriate size
;     ask groupMembers ? [
;       set size 2
;     ]
   ]
   ;; update the active groups list
   set activeGroups (filter is-active? groups)
end

to-report proportionalSize [g]
  let a (groupSize g - mingroupsize)
  let b (maxgroupsize - mingroupsize)
  
  report a / b  
end

to-report getGroup [gi]
  let x position gi (map groupId groups)
  report item x groups 
end


to activate [g]
;  set thickness (1 + proportionalSize (getGroup g))
  show-link
  set gids (fput g gids)
end

to deactivate 
  set gids but-first gids
  ifelse empty? gids [ 
    hide-link 
  ] [
    let h first gids
;    set thickness (1 + proportionalSize (getGroup h))
  ] 
end

to-report is-active? [g]
  let s groupStart g
  let t groupEnd g
  
  report (s <= ticks and ticks <= t) 
end
@#$#@#$#@
GRAPHICS-WINDOW
275
