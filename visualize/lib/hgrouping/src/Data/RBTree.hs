{-|
   ...
   Red-Black tree, taken from/based on: https://github.com/kazu-yamamoto/llrbtree/
...
import Prelude hiding (minimum, maximum, null,map)
...
-- |Warning, this may ruin the ordering. O(n)
map                    :: Ord b => (a -> b) -> RBTree a -> RBTree b
map f Leaf             = Leaf
map f (Node c h l x r) = Node c h (map f l) (f x) (map f r)
